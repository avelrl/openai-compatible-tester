...
func Registry() []TestCase {
	return []TestCase{
		...
		{
			ID:                   "responses.assistant_phase",
			Name:                 "Responses assistant phase",
			Category:             "responses",
			Family:               "phase",
			Target:               responsesExtendedTarget,
			Kind:                 KindResponses,
			RequiredCapabilities: []string{"responses.assistant_phase"},
			Run:                  runResponsesAssistantPhase,
		},
		{
			...
			Run:                  runResponsesCompact,
		},
		{
			ID:                   "responses.compact.missing_model",
			Name:                 "Responses compact missing model",
			Category:             "responses",
			Family:               "compaction",
			Target:               responsesExtendedTarget,
			Kind:                 KindResponses,
			RequiredCapabilities: []string{"responses.compaction"},
			Run:                  runResponsesCompactMissingModel,
		},
		{
			ID:                   "responses.websocket.basic",
			Name:                 "Responses WebSocket basic",
			Category:             "responses",
			Family:               "websocket",
			Target:               responsesExtendedTarget,
			Kind:                 KindResponses,
			RequiredCapabilities: []string{"responses.websocket"},
			Run:                  runResponsesWebSocketBasic,
		},
		{
			ID:                   "responses.websocket.sequential",
			Name:                 "Responses WebSocket sequential",
			Category:             "responses",
			Family:               "websocket",
			Target:               responsesExtendedTarget,
			Kind:                 KindResponses,
			RequiredCapabilities: []string{"responses.websocket"},
			Run:                  runResponsesWebSocketSequential,
		},
		{
			...
			Kind:                 KindResponses,
			RequiredCapabilities: []string{"responses.websocket"},
			Run:                  runResponsesWebSocketContinuation,
		},
		{
			ID:                   "responses.websocket.previous_response_not_found",
			Name:                 "Responses WebSocket missing previous_response_id",
			Category:             "responses",
			Family:               "websocket",
			Target:               responsesExtendedTarget,
			Kind:                 KindResponses,
			RequiredCapabilities: []string{"responses.websocket"},
			Run:                  runResponsesWebSocketPreviousResponseNotFound,
		},
		{
			ID:                   "responses.websocket.reconnect_store_false_recovery",
			Name:                 "Responses WebSocket store:false reconnect recovery",
			Category:             "responses",
			Family:               "websocket",
			Target:               responsesExtendedTarget,
			Kind:                 KindResponses,
			RequiredCapabilities: []string{"responses.websocket"},
			Run:                  runResponsesWebSocketReconnectStoreFalseRecovery,
		},
		{
			ID:                   "responses.websocket.failed_continuation_evicts_cache",
			Name:                 "Responses WebSocket failed continuation evicts cache",
			Category:             "responses",
			Family:               "websocket",
			Target:               responsesExtendedTarget,
			Kind:                 KindResponses,
			RequiredCapabilities: []string{"responses.websocket"},
			Run:                  runResponsesWebSocketFailedContinuationEvictsCache,
		},
		{
			ID:                   "responses.websocket.compact_new_chain",
			Name:                 "Responses WebSocket compact new chain",
			Category:             "responses",
			Family:               "websocket",
			Target:               responsesExtendedTarget,
			Kind:                 KindResponses,
			RequiredCapabilities: []string{"responses.websocket", "responses.compaction"},
			Run:                  runResponsesWebSocketCompactNewChain,
		},
		{
...
func runResponsesStream(ctx context.Context, rc RunContext) Result {
	...
	resp, err := rc.Client.PostJSONStream(ctx, rc.Config.Endpoints.Paths.Responses, payload, headers, func(ev sse.Event) error {
		...
		if shouldValidateResponsesStreamEventType(eventType) {
			appendStreamShapeErrors(&result, validateResponsesStreamEventShape(ev.Data, false))
		}
		if delta != "" {
